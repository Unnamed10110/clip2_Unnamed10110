...
std::wstring ClipboardItem::GetFileType(UINT fmt) {
    if (fmt == CF_TEXT || fmt == CF_UNICODETEXT || fmt == CF_OEMTEXT) return L"Text";
    ...
    if (fmt == CF_HDROP) {
        ...
        if (dropData && dropData->size() >= sizeof(DROPFILES)) {
            ...
            std::wstring ext = firstFile.substr(firstFile.find_last_of(L".") + 1);
    ...
    if (fmt == CF_RIFF || fmt == CF_WAVE) return L"Audio";
...
std::wstring ClipboardItem::GetPreview(const std::vector<BYTE>& data, UINT fmt) {
    if (fmt == CF_UNICODETEXT && data.size() >= sizeof(wchar_t)) {
        size_t len = (data.size() / sizeof(wchar_t)) - 1;
        if (len > 50) len = 50;
        std::wstring preview((wchar_t*)data.data(), len);
        if (data.size() / sizeof(wchar_t) > 50) preview += L"...";
        return preview;
    }
    if (fmt == CF_TEXT && data.size() > 0) {
        size_t len = data.size() - 1;
        if (len > 50) len = 50;
        std::string preview((char*)data.data(), len);
        if (data.size() > 50) preview += "...";
        return std::wstring(preview.begin(), preview.end());
    }
    const std::vector<BYTE>* dropData = GetFormatData(CF_HDROP);
    if (fmt == CF_HDROP && dropData && dropData->size() >= sizeof(DROPFILES)) {
        DROPFILES* df = (DROPFILES*)dropData->data();
        wchar_t* files = (wchar_t*)((BYTE*)dropData->data() + df->pFiles);
        std::wstring firstFile = files;
...
HBITMAP ClipboardItem::CreateBitmapFromData() {
    ...
    if (dibData && (dibFormat == CF_DIB || dibFormat == CF_DIBV5)) {
        // Create bitmap from DIB data
        BITMAPINFOHEADER* bmi = (BITMAPINFOHEADER*)dibData->data();
        
        // Calculate bits pointer (accounting for color table)
        int colorTableSize = 0;
        if (bmi->biBitCount <= 8) {
            colorTableSize = (bmi->biClrUsed ? bmi->biClrUsed : (1 << bmi->biBitCount)) * sizeof(RGBQUAD);
        }
        void* bits = (BYTE*)dibData->data() + sizeof(BITMAPINFOHEADER) + colorTableSize;
        
        hBitmap = CreateDIBitmap(hdcScreen, bmi, CBM_INIT, bits, (BITMAPINFO*)bmi, DIB_RGB_COLORS);
    }
...
void ClipboardItem::GenerateThumbnail() {
    if (format == CF_BITMAP || format == CF_DIB || format == CF_DIBV5) {
        ...
        if (hBitmap) {
            ...
            HBITMAP hThumb = CreateCompatibleBitmap(hdcScreen, thumbSize, thumbSize);
            SelectObject(hdcThumb, hThumb);
        ...
        if (dropData && dropData->size() >= sizeof(DROPFILES)) {
            ...
            if (SUCCEEDED(SHCreateItemFromParsingName(firstFile.c_str(), nullptr, IID_PPV_ARGS(&pImageFactory)))) {
                HBITMAP hBitmap = nullptr;
                SIZE size = {48, 48};
                if (SUCCEEDED(pImageFactory->GetImage(size, SIIGBF_THUMBNAILONLY, &hBitmap))) {
                    thumbnail = hBitmap;
                }
...
HBITMAP ClipboardItem::GetPreviewBitmap() {
    // Return existing preview or generate new one
    if (previewBitmap) {
        return previewBitmap;
    }
    
    if (format == CF_DIB || format == CF_DIBV5) {
        // Generate larger preview from original data
        HBITMAP hBitmap = CreateBitmapFromData();
        if (hBitmap) {
            previewBitmap = hBitmap;
            return previewBitmap;
        }
    } else if (format == CF_HDROP && (isImage || isVideo)) {
        // For image/video files, get larger thumbnail
        const std::vector<BYTE>* dropData = GetFormatData(CF_HDROP);
        if (dropData && dropData->size() >= sizeof(DROPFILES)) {
            DROPFILES* df = (DROPFILES*)dropData->data();
            wchar_t* files = (wchar_t*)((BYTE*)dropData->data() + df->pFiles);
            ...
            if (SUCCEEDED(SHCreateItemFromParsingName(firstFile.c_str(), nullptr, IID_PPV_ARGS(&pImageFactory)))) {
                HBITMAP hBitmap = nullptr;
                SIZE size = {300, 300};  // Larger size for preview
                if (SUCCEEDED(pImageFactory->GetImage(size, SIIGBF_BIGGERSIZEOK, &hBitmap))) {
                    previewBitmap = hBitmap;
                    pImageFactory->Release();
                    return previewBitmap;
                }
...
ClipboardManager::ClipboardManager() 
    : hwndMain(nullptr), hwndList(nullptr), hwndPreview(nullptr), hwndSearch(nullptr), isRunning(false), listVisible(false), lastSequenceNumber(0), hKeyboardHook(nullptr), scrollOffset(0), itemsPerPage(10), numberInput(L""), searchText(L""), isPasting(false), previousFocusWindow(nullptr), hoveredItemIndex(-1), selectedIndex(0), originalSearchEditProc(nullptr) {
    instance = this;
...
void ClipboardManager::ProcessClipboard() {
    ...
    for (int retry = 0; retry < MAX_RETRIES; retry++) {
        ...
        try {
                ...
                if (hMem) {
                    ...
                    if (size > 0 && size < 100 * 1024 * 1024) { // Limit to 100MB
                        ...
                        if (pMem) {
                            ...
                            // Create clipboard item with primary format
                            auto item = std::make_unique<ClipboardItem>(primaryFormat, data);
                            
                            // Store ALL available formats to preserve formatting
                            UINT format = EnumClipboardFormats(0);
                            while (format != 0) {
                                if (format != primaryFormat) {  // Already stored primary format
                                    HGLOBAL hFormatMem = GetClipboardData(format);
                                    if (hFormatMem) {
                                        SIZE_T formatSize = GlobalSize(hFormatMem);
                                        if (formatSize > 0 && formatSize < 100 * 1024 * 1024) { // Limit to 100MB
                            ...
                            if (!clipboardHistory.empty()) {
                                ...
                                if (item->formats.size() == lastItem->formats.size()) {
                                    ...
                                    for (const auto& formatPair : item->formats) {
                                        ...
                                        if (!lastData || lastData->size() != newData.size()) {
                                            allFormatsMatch = false;
                                            break;
                                        }
                                        
                                        // Compare data byte-by-byte
                                        if (memcmp(newData.data(), lastData->data(), newData.size()) != 0) {
                                            allFormatsMatch = false;
                                            break;
                                        }
                                    }
                            ...
                            if (!isDuplicate) {
                                ...
                                // Limit to MAX_ITEMS
                                if (clipboardHistory.size() > MAX_ITEMS) {
            ...
            // Error occurred, close clipboard and retry
            CloseClipboard();
        }
    }
}
